...
class sBertClassifier(nn.Module):
    def __init__(self, model, hidden, out_dim):
        super(sBertClassifier, self).__init__()

        self.model = model
        self.hidden = hidden
        self.classifier = nn.Sequential(
            nn.Linear(768, self.hidden),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(self.hidden, out_dim)
        )

    def forward(self, data, attention_mask, token_type_ids):

        layer, dim = self.model(
            data, attention_mask=attention_mask, token_type_ids=token_type_ids)
        last_hidden_state = layer[:, 0, :]
        #last_hidden_state = dim
